...
GitHub repo, then deploy at share.streamlit.io (connect the repo,
point at app.py).
"""
...
_state = {"activations": None, "gradients": None}
...
def predict_with_gradcam(image_rgb: np.ndarray, threshold_pct: int):
    ...
    is_uncertain = confidence < threshold_pct / 100.0
    label = clean_label(CLASS_NAMES[class_idx])
    per_class = {clean_label(name): float(probs[i]) for i, name in enumerate(CLASS_NAMES)}

    return label, confidence, is_uncertain, overlay, per_class
...
st.markdown(
    ...
    This model was trained to **99.2% validation accuracy** (0.9997 ROC-AUC) and
    compiled/optimized for the **Snapdragon X Elite** via Qualcomm AI Hub for the
    original desktop application. This web demo runs standard CPU inference for
    public sharing — see the
...
with col1:
    uploaded_file = st.file_uploader("Upload a bread image", type=["png", "jpg", "jpeg"])
    threshold = st.slider("Confidence threshold (%)", 50, 99, 70)
...
with col2:
    if uploaded_file is not None:
        ...
        with st.spinner("Running inference..."):
            label, confidence, is_uncertain, overlay, per_class = predict_with_gradcam(image_rgb, threshold)

        if is_uncertain:
            st.warning(f"⚠️ UNCERTAIN — best guess: {label} ({confidence*100:.1f}%)")
        elif label == "Good":
            st.success(f"✅ GOOD — {confidence*100:.1f}% confidence")
        else:
            st.error(f"❌ DEFECTIVE — {confidence*100:.1f}% confidence")

        st.bar_chart(per_class)

        st.image(overlay, caption="Grad-CAM Explainability Heatmap", use_column_width=True)
        st.caption("Warmer colors (red/yellow) show the regions that most influenced this result.")
    else:
        st.info("Upload an image on the left to run an inspection.")
